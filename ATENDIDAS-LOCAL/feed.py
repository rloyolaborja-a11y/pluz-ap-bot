...
def _publicar_commit_github(cfg, archivos, mensaje):
    ...
    vacío (sin ningún commit todavía): la Git Data API entera no funciona
    hasta que exista un primer commit -- se usa la Contents API para ese
    arranque puntual, con el primer archivo de esta publicación."""
...
def _leer_puntero(cfg):
    ...
    lectura pública, sin token) y si no hay nada ahí cae al doGet de la
    Apps Script de Google. Si no existe todavía o algo falla en los dos, se
...
def publicar_lote(cfg, archivos, mensaje):
    ...
    de Apps Script/Drive -- por si algun dia es al reves (GitHub bloqueado,
    Google no) en otra PC/red."""
    if _GIT_DISPONIBLE:
        try:
...
def main():
    ...
    # Índice de la pestaña Correcciones (casos "No se encuentra" + los ya
    # corregidos, de TODOS los meses) — un solo archivo chico en vez de que
    # el sitio baje todos los meses completos.
    ruta_indice = os.path.join(SALIDA_DIR, "correcciones_indice.json")
    if os.path.exists(ruta_indice):
        with open(ruta_indice, "rb") as f:
            contenido_indice = f.read()
        n_idx = len(json.loads(contenido_indice).get("casos", []))
        path_indice_slot = _con_slot(cfg["path"].replace("bd_actual.json", "correcciones_indice.json"), slot_nuevo)
        print(f"Índice de Correcciones: {n_idx} caso(s) ({path_indice_slot})")
        archivos.append((path_indice_slot, contenido_indice))
